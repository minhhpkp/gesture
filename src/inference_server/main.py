...
from utils.misc import load_config, make_logger, set_seed, get_logger
from worker import Worker
from buffers import DropOldBuffer, DropOldBatchBuffer
from det import DetectionInferencer
from pose import PoseInferencer, get_part_keypoints_count, get_part_keypoints
...
class DetectionInferenceWorker(Worker):
    def __init__(self, frame_buf: DropOldBatchBuffer, det_buf: DropOldBuffer, batch_size: int, batching_timeout: float,
                 height: int, width: int, inferencer: DetectionInferencer, daemon = True):
        super().__init__(daemon=daemon)
        self.logger = get_logger()
        self.inferencer = inferencer
        self.inferencer.warm_up(batch_size=batch_size, height=height, width=width, steps=10)
    ...
    def work(self):
        frames = self.frame_buf.get_batch(self.batch_size, timeout=self.batching_timeout)
        
        np_frames = [self.resize(frame[0]) for frame in frames]

        results = self.inferencer.infer(np_frames)
        if len(results) != len(frames):
            self.logger.error('det: unexpected frame drops occurred')
        
        for i, result in enumerate(results):
            self.det_buf.put((np_frames[i], result, frames[i][1]))        

    def resize(self, frame) -> np.ndarray:
        """
        Preprocess CV2 frames.

        Argument:
        --- 
            frame: RBG uint8 HxWx3
        ...
        img = np.ascontiguousarray(img)          # important to avoid hidden copies        
        return img

class PostProcessingWorker(Worker):
    def __init__(self, slots: List[Slot], scheduled_ids: queue.Queue, free_ids: queue.Queue, loop: asyncio.AbstractEventLoop, out_buf: "asyncio.Queue[int]",
                 logits2glosses: Callable[[torch.Tensor], List], voting_bag_size: int,  blank_id: int, voting_bag_stride: int = 1, voting_strategy: VotingStrategy = VotingStrategy.MAJORITY, daemon = True):
    ...
    def work(self):
        ...
        if self.voting_strategy == VotingStrategy.MAJORITY:          
            glosses = self.prev_res + self.logits2glosses(slot.logit_cpu)
            logger.info(f'post: glosses={glosses} delay={time.monotonic()-slot.start}')
            for start in range(0, N - self.B + 1, self.S):
                if self.B == 1: # Naive greedy
                    gloss_id = glosses[start]
                else:
    ...
    def _save_result(self, gloss_id: int):
        if gloss_id != self.blank_id and (gloss_id != self.last_output_id or self.last_voting_result == self.blank_id):
            self.loop.call_soon_threadsafe(self.out_buf.put_nowait, gloss_id)
            self.last_output_id = gloss_id
        self.last_voting_result = gloss_id
...
# det_inf: "Optional[DetectionInferencer]" = None
# pose_inf: "Optional[PoseInferencer]" = None
# cslr_inf: "Optional[OnlineCSLRInferencer]" = None
...
async def lifespan(app: FastAPI):
    ...
    data_cfg = cfg['data']
    used_parts = data_cfg['use_keypoints']
    window_size = data_cfg.get('win_size', 16)    
    stride = data_cfg.get('stride', 1)
    win_batch_size = data_cfg.get('split_size', 2)  
    voting_bag_size = 1
    voting_strat = VotingStrategy.MAJORITY
    width = 210
    height = 260
    det_batch_size = data_cfg.get('det_batch_size', 2)
    num_slots = data_cfg.get('slots', 1)

    model_dir = cfg['training']['model_dir']
    global logger
    logger = make_logger(model_dir=model_dir, log_file='inference_server.log')
    keypoint_num = get_part_keypoints_count(used_parts)

    det_inf = DetectionInferencer(det_ckpt=cfg['det']['ckpt'], det_config=cfg['det']['config'], device='cpu')    
    pose_inf = PoseInferencer(pose_ckpt=cfg['pose']['ckpt'], pose_config=cfg['pose']['config'], device='cpu')
    cslr_inf = OnlineCSLRInferencer(cfg, ckpt_name)
    vocab_size = len(cslr_inf.vocab)
    config = Config(
        window_size=window_size,
        stride=stride,
        win_batch_size=win_batch_size,
        voting_bag_size=voting_bag_size,
        keypoint_num=keypoint_num,
        used_parts=used_parts,
        voting_strat=voting_strat,
        width=width,
        height=height,
...
@app.websocket("/infer")
async def infer(websocket: WebSocket):    
    await websocket.accept()

    # Per-connection resources
    frame_buf = DropOldBatchBuffer(maxsize=1000000)
    det_buf = DropOldBuffer(maxsize=1000000)
    det_worker = DetectionInferenceWorker(
        frame_buf=frame_buf, det_buf=det_buf, 
        batch_size=config.det_batch_size,
        batching_timeout=1000.0, 
        height=config.height, width=config.width, inferencer=det_inf)

    pose_buf = DropOldBatchBuffer(maxsize=1000000)
    pose_worker = PoseInferenceWorker(
        det_buf=det_buf, pose_buf=pose_buf,
        used_parts=config.used_parts, height=config.height, width=config.width, vis_buf=None, inferencer=pose_inf)
    slots = [Slot(
        keypoint_num=config.keypoint_num, height=config.height, width=config.width,
        window_size=config.window_size, stride=config.stride, max_batch_size=config.win_batch_size,
        vocab_size=config.vocab_size, device=config.device) for _ in range(config.num_slots)]
    free_ids = queue.Queue()
    for id in range(config.num_slots):
        free_ids.put(id)
    loaded_ids = queue.Queue()
    scheduled_ids = queue.Queue()

    win_batcher = WindowBatcher(
        pose_buf=pose_buf, window_size=config.window_size, stride=config.stride, 
        max_batch_size=config.win_batch_size, batching_timeout=1000.0, slots=slots, 
        free_ids=free_ids, loaded_ids=loaded_ids)

    cslr_worker = CSLRInferenceWorker(
        inferencer=cslr_inf, slots=slots, loaded_ids=loaded_ids,
        scheduled_ids=scheduled_ids, max_batch_size=config.win_batch_size)

    loop = asyncio.get_running_loop()
    out_buf = asyncio.Queue(maxsize=1000000)
    def logits2glosses(logits: torch.Tensor) -> torch.Tensor:
        res = cslr_inf.predict_gloss_from_logits(logits, k=1)
        return res.squeeze(1).tolist()
    
    postproc_worker = PostProcessingWorker(
        slots=slots, scheduled_ids=scheduled_ids, free_ids=free_ids, loop=loop,
        out_buf=out_buf, logits2glosses=logits2glosses, voting_bag_size=config.voting_bag_size,
        blank_id=cslr_inf.blank_id, voting_strategy=config.voting_strat)
    
    async def sender_loop() -> None:
        """
        Single sender task => no concurrent ws.send_json calls.
        """
        while True:
            output = await out_buf.get()
            try:
                await websocket.send_json({'gloss_id': output})
            finally:
                out_buf.task_done()
    ...
    win_batcher.start()
    cslr_worker.start()
    postproc_worker.start()
    sender_task = asyncio.create_task(sender_loop())

    postproc_worker.start_work()
    cslr_worker.start_work()
    win_batcher.start_work()
    pose_worker.start_work()
    det_worker.start_work()
    ...
    try:        
        while True:    
            ...
            print(f"Received {len(image_bytes)} bytes, {width} x {height}")

            arr = np.frombuffer(image_bytes, dtype=np.uint8)
            arr = arr.reshape((height, width, 3))

            # tmp = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
            # cv2.imwrite(f'vis_lk/{idx}.png', tmp)
            # idx = (idx + 1) % 10
            
            frame_buf.put((arr, time.monotonic()))
    except WebSocketDisconnect:
        print('USER DISCONNECTED')
    finally:
        # Stop worker threads
        det_worker.stop_work()
        pose_worker.stop_work()
        win_batcher.stop_work()
        cslr_worker.stop_work()
        postproc_worker.stop_work()
        ...
        await loop.run_in_executor(None, det_worker.join, 1.0)
        await loop.run_in_executor(None, pose_worker.join, 1.0)
        await loop.run_in_executor(None, win_batcher.join, 1.0)
        await loop.run_in_executor(None, cslr_worker.join, 1.0)
        await loop.run_in_executor(None, postproc_worker.join, 1.0)
        print('CLEANUP DONE')