...
class TritonPythonModel:
    ...
    def auto_complete_config(auto_complete_model_config: pb_utils.ModelConfig):
        ...
        inputs = [{
            ...
            'data_type': 'TYPE_FP32',
            'dims': [-1, 4]             # N, 4 (only box coordinates)
        }, {
        ...
        outputs = [{
            'name': 'KEYPOINTS',        # keypoints prediction for each detection
            'data_type': 'TYPE_FP32',
            'dims': [-1, -1, 3]         # N, K, 3
        }]
    ...
    def execute(self, requests):
        ...
        for i, request in enumerate(requests):
            ...
            all_images.append(image_input.as_numpy())
            bboxes_flattened = pb_utils.get_input_tensor_by_name(request, 'BBOXES').as_numpy()
            bbox_counts = pb_utils.get_input_tensor_by_name(request, 'BBOX_COUNTS').as_numpy().tolist()

            offset = 0
            for count in bbox_counts:
                # bboxes of each image
                bboxes = bboxes_flattened[offset:offset+count]  # count, 4
                all_bboxes.append(bboxes)
                offset += int(count)

        batched_images = np.concatenate(all_images, axis=0)
        batched_output = self.detector.batch(batched_images, all_bboxes)
        ...
        for i in range(num_requests):
            ...
            cur_idx += 1
            offset += batch_size
            keypoints_flattened = np.concatenate(individual_output, axis=0)
            out_tensor = pb_utils.Tensor("KEYPOINTS", keypoints_flattened)
            responses[i]  = pb_utils.InferenceResponse(output_tensors=[out_tensor])
