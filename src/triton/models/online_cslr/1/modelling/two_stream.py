...
class S3D_two_stream_v2(nn.Module):
    # use pyramid v2
    def __init__(self, use_block=5, freeze_block=(0,0), pose_inchannels=17, flag_lateral=[False, False], pretrained_ckpt='pretrained_models/s3ds_actioncls_ckpt', **kwargs):
        ...
        self.rgb_stream = S3D_backbone(3, use_block, freeze_block[0], cfg_pyramid=self.cfg_pyramid, use_shortcut=self.use_shortcut, pretrained_ckpt=pretrained_ckpt)
        self.pose_stream = S3D_backbone(pose_inchannels, use_block, freeze_block[1], cfg_pyramid=self.cfg_pyramid, 
                                        coord_conv=kwargs.pop('coord_conv', None), use_shortcut=self.use_shortcut, pretrained_ckpt=pretrained_ckpt)
