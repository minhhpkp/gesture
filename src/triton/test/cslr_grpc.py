...
def main():
    ...
    def handle_output(gloss_id: int):
        ...
        if gloss_id != BLANK_ID and (last_output_id != gloss_id or last_accepted_id == BLANK_ID):
            print(vocab[gloss_id], end=' ', flush=True)
    ...
    def batched_clsr_inference():
        ...
        # print(f'batch keypoints shape = {batched_window_keypoints.shape}')
        ...
        cslr_resp = client.infer(model_name='online_cslr', inputs=[inp_clips, inp_keypoints], outputs=[out_gloss])
        gls_logits = cslr_resp.as_numpy('GLOSS')
        # print(f'cslr shape = {gls_logits.shape}')
        
        # naive greedy decoding
        gls_indices = np.argmax(gls_logits, axis=-1).tolist()

        glosses = [vocab[idx] for idx in gls_indices]
        print(f' {' '.join(glosses)}', end='', flush=True)
