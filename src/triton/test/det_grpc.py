...
def main():
    ...
    for begin in range(0, imgs_np.shape[0], args.batch_size):
        ...
        det_resp = client.infer(model_name='det', inputs=[inp_imgs], outputs=[out_bboxes, out_bbox_counts])
        bboxes = det_resp.as_numpy('BBOXES')
        bbox_counts = det_resp.as_numpy('BBOX_COUNTS').tolist()
